...
public interface PantsDao extends BaseMapper<Pants> {
    ...
    @Select("SELECT c.*, cl.* FROM pants c JOIN clothing cl ON c.clothing_id = cl.id AND cl.isactive>-1 WHERE c.id = #{id}")
    @Results({
    ...
    @Select("<script>" +
            "SELECT c.*, cl.* FROM pants c JOIN clothing cl ON c.clothing_id = cl.id AND cl.isactive>-1 AND cl.userid=#{userid}" +
    ...
    @Select("SELECT c.*, cl.* FROM pants c JOIN clothing cl ON c.clothing_id = cl.id AND cl.isactive>-1 ${ew.customSqlSegment}")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "clothing", column = "clothing_id", one = @One(select = "com.closet.onlinecloset.dao.ClothingDao.selectById"))
    })
