...
public interface UnderWearDao extends BaseMapper<UnderWear> {
    ...
    @Select("SELECT c.*, cl.* FROM underwear c JOIN clothing cl ON c.clothing_id = cl.id WHERE c.id = #{id}")
    @Results({
    ...
    @Select("<script>" +
            "SELECT c.*, cl.* FROM underwear c JOIN clothing cl ON c.clothing_id = cl.id AND cl.type>-1" +
